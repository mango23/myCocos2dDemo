...
Utils.formData = function (o) {
    ...
    for (var k in o) {
      if (o[k]) {
        kvps.push(Utils.encode(k).replace(regEx, "+") + "=" + Utils.encode(o[k].toString()).replace(regEx, "+"));
      }
    }
    return kvps.join('&');
};
...
Utils.ajax = function (o) {
    ...
    var url = o.root + '' + o.url, data = null;
    var isPost = o.type == "POST" || o.type == "PUT";
    if (window.ttsid == null && !o.data.ajax) {
      return
    } else {
      o.data.ttsid = window.ttsid
    }
    if (o.data && typeof o.data == 'object') {
      o.data.openid = window.openid
      o.data.openkey = window.openkey
      console.log(o.data)
...
Utils.get = function (opt) {
    if (Utils.isFunction(opt.data)) {
        opt.success = opt.data;
        opt.data = {};
    }
    Utils.ajax({root: opt.root, url: opt.url, type: "GET", data: opt.data, success: opt.success, error: opt.error });
};

Utils.post = function (opt) {
    if (Utils.isFunction(opt.data)) {
        opt.success = top.data;
        opt.data = {};
    }
    Utils.ajax({root: opt.root, url: opt.url, type: "POST", data: opt.data, success: opt.success });
};
